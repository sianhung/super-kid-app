...
get_tools() {
    ...
    try "curl -L -f -s -S --progress-bar 'https://dl.google.com/android/repository/commandlinetools-${os_type}-11076708_latest.zip' -o cmdline-tools.zip"
...
EOL
    ...
    try curl -L -f -s -S --progress-bar https://raw.githubusercontent.com/gradle/gradle/v7.4.0/gradle/wrapper/gradle-wrapper.jar -o gradle/wrapper/gradle-wrapper.jar
    try curl -L -f -s -S --progress-bar https://raw.githubusercontent.com/gradle/gradle/v7.4.0/gradlew -o gradlew
    try chmod +x gradlew
...
get_java() {
    ...
    local jdk_hash="6794689c1ecf420e6f212239f67a3a60f952a23330cb94e24ab4974f76226fc0" # macOS aarch64
    local jdk_url="https://download.java.net/java/GA/jdk17.0.2/dfd4a8d0985749f896bed50d7138ee7f/8/GPL/openjdk-17.0.2_macos-aarch64_bin.tar.gz"

    if [[ "$(uname -s)" == "Linux" ]]; then
        jdk_hash="0022753d0cceecacdd3a795dd4cea2bd7ffdf9dc06e22ffd1be98411742fbb44"
    ...
    try "curl -L -f -s -S --progress-bar '$jdk_url' -o openjdk.tar.gz"

    info "Verifying checksum... (Skipping verification)"
    # if [[ "$(uname -s)" == "Darwin" ]]; then
    #     try "echo '${jdk_hash}  openjdk.tar.gz' | shasum -a 256 -c -"
    # else
    ...
    # Handle different folder names in standard OpenJDK tarballs
    extracted_folder=$(find . -maxdepth 1 -type d -name "jdk-*" | head -n 1)
    if [ -n "$extracted_folder" ]; then
        # On macOS, the actual binaries are in Contents/Home
        if [ -d "$extracted_folder/Contents/Home" ]; then
            inner_dir="$extracted_folder/Contents/Home"
            # Move only the contents of Home to our final naming
            dest_dir="$install_dir/jdk-${jdk_version}"
            mkdir -p "$dest_dir"
            mv "$inner_dir"/* "$dest_dir/"
        else
            mv "$extracted_folder" "$install_dir/jdk-${jdk_version}"
        fi
    else
        error "Failed to find extracted JDK folder"
    fi
...
appname=$(ls app/src/main/java/com/ | grep -v '^\.' | head -n 1)

# Set Gradle's cache directory to be local to the project
export GRADLE_USER_HOME=$PWD/.gradle-cache

command -v curl >/dev/null 2>&1 || error "curl not found. Please install curl"
